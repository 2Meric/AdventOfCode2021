...
def find_paths(caves : str) -> [[int]]:
    ...
    while packets:
        ...
        path = packets.pop()
        i = dests.pop()
        s_vi = small_visit.pop()

        print(caves[i].get_name() + ': ' + str([caves[idx].get_name() for idx in path]) + ' - ' + str(1 if s_vi else 0))
        ...
        N = caves[i].get_N()
        for n in N:
            # set small visited to the packets
            s_v = s_vi

            # if the destination is the start node --> we cannot go back to start
            if n == start:
                continue
            ...
            # if neighbour is a small cave we already visited
            if contains(path, n) and not caves[n].is_big():
                # we can visit 1 small cave twice
                if not s_v:
                    print('s_v at ' + caves[n].get_name())
                    s_v = True
                else:
                    continue
